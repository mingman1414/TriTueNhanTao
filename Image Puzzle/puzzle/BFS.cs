...
namespace A_BFS
{
	class State
	{
		...
		public State(List<int> trangThaii)
		{
			this.trangThai = trangThaii;
		}
	...
	class BFS//chieu rong
	{
        ...
        public BFS(State trangThaiDauu, State trangThaiDichh)
		{
			this.trangThaiDau = trangThaiDauu;
			this.trangThaiDich = trangThaiDichh;
		...
		public List<State> Solve()//solve chieu rong
		{
			dem = 0;
			...
			while (danhSachDinhDuyet.Count > 0)
			{
				...
				List<State> PhanTach = ptLayRa.PhanTachTrangThai();
				foreach (var item in PhanTach)
				{
					if (item.KiemTraDenDich(trangThaiDich) == true)
					{
						Console.WriteLine("Ban da chien thang!.....");
						//lằ vết lúc này sẽ trả về danh sách đường đi nhưng bị ngược => qua nút Giải reverse nó
						LanVet(ketQua, item);
		...
		{
			State hienTai = n;
			duongDi.Add(hienTai); //ketqua.Add(hienTai);
